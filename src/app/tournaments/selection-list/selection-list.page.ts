  ...
  ngOnInit() {
    ...
    this.routeSub = this.route.paramMap.subscribe((paramMap: ParamMap) => {
      ...
      if (paramMap.has('playerId')) {
        ...
        const player = this.playersService.getOnePlayer(this.playerId);

        if (player) {
          this.navigationService.setNavLink(player.firstName + ' ' + player.lastName);
        } else {
          this.navigationService.setNavLink('Tournament Selection');
        }
        
        

        // get selected tournament for this specific player
        this.selectedTournaments = this.registrationsService.selectedTournaments.filter(element => element.playerId ===  this.playerId);
        ...
        this.tournamentsService.getTournaments()
          .subscribe(tournaments => {
            ...
            tournaments.forEach(tournament => {
              ...
              const t = this.selectedTournaments.find(selection => ( tournament._id.toString() === selection.tournamentId.toString()));
              if (t !== undefined) {
                // match, tournament was already selecetd, thus setting isSelecetd to true
                this.playerTournaments.push( {
                  ...tournament, isSelected: true, isLocal: t.isLocal
                });
              } else {
                this.playerTournaments.push( {
                  ...tournament, isSelected: false, isLocal: true
                });
              }
